...
def make_train_data(cube_env, net, device, batch_size, scramble_depth, shuffle=True):
    ...
    for idx, s in enumerate(cube_states):
        ...
        goal = np.argmax(goals) if any(goals) else None
        goal_indices.append(goal)
    ...
    value_t = torch.clamp(value_t, -np.inf, 1.0)
    max_val_t, max_act_t = value_t.max(dim=1)

    for idx, goal_idx in enumerate(goal_indices):
        if goal_idx is not None:
            max_val_t[idx] = 1.0
            max_act_t[idx] = int(goal_idx)

    # add reward to the values
#    goals_mask_t = torch.tensor(explored_goals, dtype=torch.float32).to(device)
#    value_t = torch.max(value_t, goals_mask_t)
